...
let dia_help =
  "Dia: The brilliance of Diamond, the elegance of the Language\n\n\
   ...
   --static      Generate static binary (equivalent of --static option in C++ \
   compilers)\n\n\
...
let speclist =
  [
    ( "-v",
      Arg.Set cli_options.verbosity,
      "Increase the level of verbosity (-v to -vvvv)" );
    ( "-o",
      Arg.Set_string cli_options.output_file,
      "Set explicit output file name." );
    ("--stdout", Arg.Set cli_options.std_out, "Print compiled code to terminal");
    ( "--no-compile",
      Arg.Set cli_options.no_compile,
      "Do not compile, persist the source code." );
    ( "--skip-format",
      Arg.Set cli_options.skip_format,
      "Skip formatting the generated code using clang-format." );
    ("--quiet", Arg.Set cli_options.quiet, "Do not generate file comment.");
    ( "--static",
      Arg.Set cli_options.static,
      "Generate static binary (equivalent of --static option in C++ compilers)"
    );
    (* Experimental Options *)
    ( "--pure",
      Arg.Set cli_options.pure,
      "Add [[gnu::pure]] macro to all custom functions." );
    ( "--constexpr",
      Arg.Set cli_options.constexpr,
      "Add constexpr keyword to all custom functions." );
    ( "--faster-io",
      Arg.Set cli_options.faster_io,
      "Disable input and output buffer to increase I/O performance.\n\
       (Should be used on competitive programming)" );
  ]

let _ =
  let () =
    Arg.parse speclist (fun str -> cli_options.input_file := str) dia_help
  in
  let ch =
    if cli_options.input_file.contents = "" then
      raise
        (DiaFileNotFoundError
           "Dia: No input file given. Example: diac <file>.dia")
  ...
  List.iter
    (fun f -> dia_custom_function f result.custom_functions)
    result.custom_functions;
