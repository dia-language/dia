...
let generate_comment (orig_code: string) =
  let list_bulleted_orig_code =
    String.trim orig_code
    |> String.split_on_char '\n'
    |> List.map (String.cat "* ") in
  let commented_orig_code =
    List.fold_left (fun x acc -> x ^ "\n" ^ acc)
...
let dia_custom_function (node: DiaNode.dia_node) (custom_functions: DiaNode.dia_node list) =
  match node.next_function with
  | None ->
