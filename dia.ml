...
let dia_dbgprint str = Printf.eprintf "[dia.ml] %s\n" str

let rec treemaker chr depth =
  match depth with 0 -> " " | _ -> chr ^ treemaker chr (depth - 1)
...
let rec dia_debug_function_descriptor (node : DiaNode.dia_node) depth : unit =
  ...
  Printf.eprintf "%stoken_type: %s\n" (treemaker "-" depth)
    (debug_type_to_string node.token_type);
  Printf.eprintf "%snum_of_parameters: %d\n" (treemaker "-" depth)
    node.num_of_parameters;
  Printf.eprintf "%sParameters:\n" (treemaker "-" depth);
  List.iter
    (fun e -> dia_debug_function_descriptor e (depth + 1))
    node.parameters
...
let generate_comment (orig_code : string) =
  let list_bulleted_orig_code =
    String.trim orig_code |> String.split_on_char '\n'
    |> List.map (String.cat "* ")
  in
  let commented_orig_code =
    List.fold_left
      (fun x acc -> x ^ "\n" ^ acc)
      (List.nth list_bulleted_orig_code 0)
      (List.drop 1 list_bulleted_orig_code)
  in
...
let rec dia_generate_code (node : DiaNode.dia_node)
    (custom_functions : DiaNode.dia_node list) (var_index : int) :
    DiaNode.dia_node * int =
  if node.name = "if" then
    ...
    | DiaFunctionParam _ ->
        dia_dbgprint
          (Printf.sprintf "Function parameter '%s' detected" node.name);
        (node, var_index)
    | DiaFunction _ -> (
        dia_dbgprint
          (Printf.sprintf "Generating code for function '%s'" node.name);
        dia_debug_function_descriptor node 0;
        (* Evaluate the parameters first. *)
        let parameters, _var_index =
          dia_generate_code_parameter node.parameters custom_functions var_index
        in
        ...
        match cfunc with
        | Some func ->
            List.map (fun (p : DiaNode.dia_node) -> p.name) parameters
            |> String.concat ","
        ...
        | None -> (
            ...
            let pfunc =
              List.find_opt
                (fun (e : Predefined.dia_predefined_function) ->
            ...
            | None ->
                raise
                  (DiaSyntaxError
                     ("Error: Unknown identifier '" ^ node.name
...
and dia_generate_code_parameter (nodes : DiaNode.dia_node list)
    (custom_functions : DiaNode.dia_node list) (var_index : int) :
    DiaNode.dia_node list * int =
  match nodes with
  | [] -> ([], var_index)
  | n :: ns ->
...
and dia_if (node : DiaNode.dia_node) (custom_functions : DiaNode.dia_node list)
    (var_index : int) (if_depth : int) : DiaNode.dia_node * int * int =
  dia_dbgprint "Generating code of if-else clause";
  let _, _, _if_depth =
    dia_if_parse_condition node custom_functions var_index if_depth
  in
  dia_if_parse_bodies node custom_functions var_index (_if_depth + 1)

and dia_if_parse_condition (node : DiaNode.dia_node)
    (custom_functions : DiaNode.dia_node list) (var_index : int)
    (if_depth : int) : DiaNode.dia_node * int * int =
  let _, _var_index =
    dia_generate_code (List.nth node.parameters 0) custom_functions var_index
  in
  let else_clause = List.nth node.parameters 2 in
  match else_clause.name with
  (* else if (<Condition>) <Dia expression> *)
  | "if" ->
      dia_if_parse_condition else_clause custom_functions _var_index
        (if_depth + 1)
  (* else <Dia expression> *)
  | _ ->
      dia_dbgprint (Printf.sprintf "if_parse_condition: depth is %d" if_depth);
      (node, var_index, if_depth)

and dia_if_parse_bodies (node : DiaNode.dia_node)
    (custom_functions : DiaNode.dia_node list) (var_index : int)
    (if_depth : int) : DiaNode.dia_node * int * int =
  (* if { <body> }*)
  let _ = Printf.printf "if(%s){\n" (dia_create_cpp_variable var_index) in
  let body_var_index =
    (* The body_var_index would hold an incremented value from the latest used variable index. *)
    ...
    | DiaConstant _ ->
        ...
        var_index + 1
    | _ ->
        let _, body_var_index =
          dia_generate_code _node custom_functions (var_index + if_depth)
        in
        Printf.printf "return %s;\n} else "
          (dia_create_cpp_variable body_var_index);
        body_var_index + 1
  in
  (* else *)
  ...
  (* else if *)
  if _node.name = "if" then
    dia_if_parse_bodies _node custom_functions (var_index + 1) if_depth
  (* else { <body> }*)
    else
    let _node = List.nth node.parameters 2 in
    match _node.token_type with
    ...
    | _ ->
        ...
        let _, body_var_index =
          dia_generate_code _node custom_functions body_var_index
        in
        Printf.printf "return %s;\n}\n"
          (dia_create_cpp_variable (body_var_index - 1));
        (node, body_var_index + 1, if_depth)

let rec dia_generate_code_chain (node : DiaNode.dia_node)
    (custom_functions : DiaNode.dia_node list) (var_index : int) :
    DiaNode.dia_node * int =
...
let dia_main (node : DiaNode.dia) orig_code =
  ...
  |> String.concat ", " |> dia_dbgprint;
  print_endline generate_header;
  print_endline (generate_comment orig_code);
  let _, _ =
    dia_generate_code_chain node.main_function node.custom_functions 0
  in
...
let generate_custom_func_header (node : DiaNode.dia_node) =
  ...
  List.map
    (fun (p : DiaNode.dia_node) ->
      Printf.sprintf "%s %s" (type_to_string p.token_type) p.name)
    node.parameters
  |> String.concat "," |> print_string;
  print_endline ") {"

let dia_custom_function (node : DiaNode.dia_node)
    (custom_functions : DiaNode.dia_node list) =
  match node.next_function with
  | None ->
      Printf.sprintf
        "Function %s have no function body. Skipping generating code." node.name
      |> dia_dbgprint
  | Some body -> (
      dia_dbgprint
        (Printf.sprintf "Generating code for custom function '%s'" node.name);
      ...
      | _ ->
          let _, cpp_var_count =
            dia_generate_code_chain body custom_functions 0
          in
